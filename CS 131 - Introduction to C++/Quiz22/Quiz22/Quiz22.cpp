...
int main()
{
	...
	// Search for the ID. Complete the function call
...
int binarySearch(const int num[], int SIZE, int value)
{
	...
	while (first < last && !found)
	{
		...
		if (num[middle] == value)      // If value is found at mid
		{
			found = true;	// Set flag to true
			position = middle;   // Record the position of the value
			
		}
		else if (num[middle] > value)  // If value is in lower half
			last = middle + 1;
		else
			last = middle - 1;           // If value is in upper half
	}
	return position;
}
