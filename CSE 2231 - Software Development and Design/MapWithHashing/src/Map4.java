...
public class Map4<K, V> extends MapSecondary<K, V> {
	...
	private void createNewRep(int hashTableSize) {
		...
		for (int i = 0; i < this.hashTable.length(); i++) {
			this.hashTable.setEntry(i, new Map2<>());
		}
		this.size = 0;
	...
	public Map4() {
		...
		createNewRep(DEFAULT_HASH_TABLE_SIZE);
	...
	public final Pair<K, V> remove(K key) {
		...
		int index = mod(hashcode, tablelength);
	...
	public final V value(K key) {
		...
		V result = this.hashTable.entry(index).value(key);;
	...
	public final boolean hasKey(K key) {
		...
		if ( this.hashTable.entry(index).hasKey(key)) {
			result = true;
		}
	...
	public final int size() {
		...
		{
			result += this.hashTable.entry(i).size();
		}
