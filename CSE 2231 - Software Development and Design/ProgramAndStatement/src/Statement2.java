...
public class Statement2 extends StatementSecondary {
    ...
    private void createNewRep() {
        ...
        Sequence<Tree<StatementLabel>> subTree = this.rep.newSequenceOfTree();
        
        this.rep.assemble(root, subTree);
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        Statement2 sTemp = (Statement2) s;
        Sequence<Tree<StatementLabel>> seqTree = this.rep.newSequenceOfTree();
        StatementLabel root = this.rep.disassemble(seqTree);
        seqTree.add(pos, sTemp.rep);
        sTemp.createNewRep();
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        Statement2 statementIf = (Statement2) s1;
        Statement2 statementElse = (Statement2) s2;
        
        StatementLabel label = new StatementLabel(Kind.IF_ELSE, c);
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        children.add(0, statementIf.rep);
        children.add(1, statementElse.rep);
        this.rep.assemble(label, children);
        
        statementIf.createNewRep();
        statementElse.createNewRep();
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        statementIf.rep = children.remove(0);
        statementElse.rep = children.remove(0);
        this.createNewRep();
