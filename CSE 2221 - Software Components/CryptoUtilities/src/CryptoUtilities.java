...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        // TODO - fill in body
        // (12 , 6 = 6) , (14, 7 = 7 ) , ( 15, 12 = 3 )

        if (!m.isZero()) {
            NaturalNumber mod = new NaturalNumber2(n.divide(m));

            if (mod.compareTo(new NaturalNumber2(0)) == 0) {
                n.copyFrom(m);
            } else {
                n.copyFrom(mod);
                reduceToGCD(m, n);
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        NaturalNumber r = new NaturalNumber2(n.divide(new NaturalNumber2(2)));

        if (r.compareTo(new NaturalNumber2()) == 0) {
            result = true;
        }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (!power.isZero()) {
            ...
            NaturalNumber multiNum = new NaturalNumber2(1);

            multiNum.copyFrom(n);

            for (NaturalNumber i = new NaturalNumber2(1); i.compareTo(p) != 0; i
                    .increment()) {
                n.multiply(multiNum);
            }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber remain = new NaturalNumber2();
        NaturalNumber wNum = new NaturalNumber2(w);
        NaturalNumber nNum = new NaturalNumber2(n);

        boolean result = false;

        wNum.multiply(w);

        remain.copyFrom(wNum.divide(n));

        if (remain.compareTo(new NaturalNumber2(1)) == 0) {
            result = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 0; i < 50; i++) {
                ...
                NaturalNumber ranNum = randomNumber(nNum);

                while (ranNum.compareTo(new NaturalNumber2(1)) <= 0) {
                    ranNum = randomNumber(nNum);
                }

                isPrime = !isWitnessToCompositeness(ranNum, n);
            }
