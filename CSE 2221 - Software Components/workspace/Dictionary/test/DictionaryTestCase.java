...
public class DictionaryTestCase {
    ...
    public void MakeHeadTest() {
        ...
        SimpleWriter output = new SimpleWriter1L("HeaderTestresult.txt");
        SimpleReader inputExp = new SimpleReader1L("HeaderTest.txt");

        DictionaryProject.makeHead("HeaderTest", output);

        SimpleReader inputResult = new SimpleReader1L("HeaderTestresult.txt");

        boolean result = true;

        while (!inputExp.atEOS() && !inputResult.atEOS()) {
            String expectedStr = inputExp.nextLine();
            String resultStr = inputResult.nextLine();
