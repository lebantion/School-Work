...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        result = str1.substring(0, str1.length() - overlap) + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        boolean x = true;
        int z = 0;
        Queue<String> tempQ = new Queue1L<String>();
        ...
        if (z == 0 && !strSet.contains(str) && x) {
            strSet.add(str);
        } else if (z != 0) {
            while (tempQ.length() != 0) {
                strSet.remove(tempQ.dequeue());
            }
            strSet.add(str);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        if (text.contains("~")) {
            for (int i = 0; i < text.length(); i++) {
                ...
                if (text.charAt(i) == '~') {
                    out.println(text.substring(count, i));
                    count = i + 1;
                }
            }
            if (!text.endsWith("~")) {
                out.println(text.substring(count, text.length()));
            }
        } else {
            out.println(text);
        }
